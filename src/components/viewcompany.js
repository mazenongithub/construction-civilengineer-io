...
const getcompanyid = urlParts.at(-1);
const socket = new WebSocket(`ws://localhost:8081/company/${getcompanyid}/websocketapi`);
const getuserid = urlParts.at(3)
...
class ViewCompany extends Component {
    ...
    componentDidMount() {
            ...
            if (response.type === "company") {
                ...
                if (accounts.insert.length > 0 || accounts.update.length > 0) {
                    ...
                    for (let newaccount of newaccounts) {
                       ...
                       if(getaccount) {
                        const i = construction.getaccountkeybyid.call(this,newaccountid)
                ...
                if(accounts.delete.length > 0) {
                    for (let deleteaccount of accounts.delete) {
                        ...
                        if(getdeleteaccount) {
                            const i = construction.getaccountkeybyid.call(this,deleteaccountid)
        ...
        socket.onopen = (evt) => {
            ...
            construction.findMyCompany.call(this);
            const myuser = construction.getuser.call(this)
            const data = { type: "join", userid: getuserid };
